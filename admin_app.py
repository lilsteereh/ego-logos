...
print(f"[admin] USER='{ADMIN_USER}', PASSWORD_SET={'yes' if os.environ.get('ADMIN_PASSWORD') else 'no'}, ALLOWLIST={ADMIN_ALLOWLIST or 'disabled'}")
...
def get_db():
    if "admin_db" not in g:
        ...
        g.admin_db.execute("PRAGMA foreign_keys = ON")
    return g.admin_db
